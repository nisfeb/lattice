...
`sync-overlay.sh` maps `lib/` → both `gub/lib/` (deployed, imported by the nexus)
...
it the way grubbery registers every app, with one idempotent `%fall` row in
grubbery's own `lib/root.hoon` on-load. Re-apply it after pulling the dev's
pushes:
