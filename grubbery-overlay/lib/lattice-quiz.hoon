::  /lib/lattice-quiz: property-based testing for Hoon (QuickCheck, roughly).
::
::  VENDORED, NOT WRITTEN HERE.
::    source:  https://github.com/runtimeverification/quiz  src/lib/quiz.hoon
::    commit:  a5fed6eb31caf665cb50f04d3a910c8f6251ca1d  (2024-01-17)
::    license: see the upstream repo.
::
::  Upstream is stale: its src/sys.kelvin says [%zuse 413] and this desk is on
::  408. It is vendored rather than depended on because there is no live
::  distribution of it for 408 and it needs edits to build here.
::
::  CHANGES FROM UPSTREAM: NONE. The file below is byte-identical to the
::  upstream src/lib/quiz.hoon apart from this header comment.
::
::  That is a result, not an omission, so here is what was checked rather than
::  assumed. The 413->408 gap could plausibly have broken four things, and did
::  not break any of them:
::    - +og's arms. It uses rad/rads/raw/raws; all four are present at 408 with
::      the same [value new-core] shape.
::    - +toon. It switches on [%0 *] / [%1 *] / [%2 *]; unchanged at 408.
::    - +mong's signature ([[gat sam] sley] -> toon); unchanged at 408.
::    - the +type cases in +fill (%noun %atom %cell %face %fork %hint %hold)
::      and the (~(mint ut ...) %noun ...) call for %hold; unchanged at 408.
::  Verified end to end by /tests/lib/quiz, which asserts that a true law is
::  heeded, a false one is refuted, a CRASHING fate is refuted rather than
::  taking the runner down, a caller-supplied norn is actually used, and %drop
::  is counted. If a future kelvin does break this, that file says which part.
::
::  Deliberately NOT changed: the ~& tracing in +check and +run. It is noisy,
::  but the trace is how a refutation reports its counterexample through the
::  test runner, and there is nowhere else for it to go.
::
::  WHY IT IS WORTH VENDORING: +run slams the property under +mong, so a CRASH
::  in the code under test is a reported failure rather than a bailed test run.
::  That is the only way to assert "never crashes on arbitrary input" in-ship.
::  It does NOT catch infinite loops: +mong has no fuel counter and no step
::  limit, so a parser that spins forever hangs the ship and this runner can
::  never report it. That class needs an external wall-clock driver; see
::  scripts/hoon-hang.sh.
::
