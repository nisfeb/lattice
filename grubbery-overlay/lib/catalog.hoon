...
|%
::  Re-export the analyzer's arms so callers get catalog's OWN analysis type
::  (one compilation), instead of importing catalog-analyzer separately and
::  hitting a two-compilation nest mismatch when passing analysis to the urQL
::  generators below.
...
++  catalog-page-refresh-urql
  ...
  =/  author-update=tape
    ...
    :~  "UPDATE catalog-pages SET category = '"  acat
        "', cat-source = 'author', confidence = .0.1"
