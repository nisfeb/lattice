...
++  edit-html
  ...
  =/  tree-html=tape
    %-  zing
    ;:  weld
      `(list tape)`~["<div class=\"newbtns\"><a class=\"nf\" href=\"/apps/lattice/edit?newfolder=1\">+ folder</a><a class=\"nf\" href=\"/apps/lattice/edit\">+ file</a></div><div class=\"newbtns\"><a class=\"nf\" id=\"upfb\" href=\"#\">&#8613; files</a><a class=\"nf\" id=\"updb\" href=\"#\">&#8613; dir</a></div><input type=\"file\" id=\"upf\" multiple style=\"display:none\"><input type=\"file\" id=\"upd\" webkitdirectory style=\"display:none\"><div id=\"uppanel\" class=\"uppanel\" style=\"display:none\"><div id=\"upmsg\"></div><div class=\"upbar\"><div id=\"upfill\"></div></div><div id=\"uperr\" class=\"uperr\"></div></div><div class=\"sec\">files</div>"]
      %+  turn  tree
      |=  [px=path page=?]
      =/  segs=(list @ta)  px
  ...
  =/  main-panes=tape
    ...
    ;:  weld
      "<pre id=\"hl\" aria-hidden=\"true\"></pre><textarea id=\"src\" spellcheck=\"false\">"  code  "</textarea>"
  ...
  ;:  weld
    ...
    (edit-js nm view kind nfolder (trip into))
    vim-script
    sw-register-script
    "</body></html>"
  ==
...
::
++  edit-js
  |=  [nm=tape view=tape kind=@tas nfolder=? into=tape]
  ^-  tape
  ;:  weld
    (trip '<script>(function(){var INTO="')
    into
    (trip '";var NAME="')
    nm
    (trip '";var KIND="')
    ...
    view
    %-  trip
    '";var $=function(i){return document.getElementById(i)};var ws=$("ws");function ap(){ws.classList.toggle("nt",localStorage.edNT==="1");ws.classList.toggle("nc",localStorage.edNC==="1")}ap();$("tt").onclick=function(){localStorage.edNT=localStorage.edNT==="1"?"0":"1";ap()};$("ct").onclick=function(){localStorage.edNC=localStorage.edNC==="1"?"0":"1";ap()};var pn=$("pname");if(pn&&pn.value){var pl=pn.value.length;pn.focus();pn.setSelectionRange(pl,pl)}function trApply(){var c=[];try{c=JSON.parse(localStorage.edColl||"[]")}catch(e){}var rs=document.querySelectorAll(".tree [data-path]");for(var i=0;i<rs.length;i++){var p=rs[i].getAttribute("data-path");var hide=false;for(var j=0;j<c.length;j++){if(p.indexOf(c[j]+"/")===0){hide=true;break}}rs[i].style.display=hide?"none":"";if(rs[i].className.indexOf("fld")>=0){var cx=rs[i].querySelector(".cx");if(cx)cx.innerHTML=c.indexOf(p)>=0?"&#9656;":"&#9662;"}}}var ftgs=document.querySelectorAll(".tree .fld .ftog");for(var fi=0;fi<ftgs.length;fi++){ftgs[fi].onclick=function(){var p=this.parentNode.getAttribute("data-path");var c=[];try{c=JSON.parse(localStorage.edColl||"[]")}catch(e){}var k=c.indexOf(p);if(k>=0)c.splice(k,1);else c.push(p);localStorage.edColl=JSON.stringify(c);trApply()}}trApply();var mt=document.querySelectorAll(".mtabs button");for(var mi=0;mi<mt.length;mi++){mt[mi].onclick=function(){var v=this.getAttribute("data-mv");ws.setAttribute("data-mv",v);for(var mj=0;mj<mt.length;mj++){mt[mj].className=mt[mj].getAttribute("data-mv")===v?"on":""}if(v==="prev")prev()}}var st=function(t,ok){var s=$("st");s.textContent=t;s.style.color=ok?"#27ae60":"#c0392b"};var ta=$("src");if(ta){ta.addEventListener("keydown",function(e){if(e.key==="Tab"){e.preventDefault();var s=ta.selectionStart;ta.value=ta.value.slice(0,s)+"  "+ta.value.slice(ta.selectionEnd);ta.selectionStart=ta.selectionEnd=s+2}})}var prev=function(){var p=$("prev");if(!p)return;if(CONTENT){fetch("/apps/lattice/page-preview?type="+KIND,{method:"POST",body:ta.value}).then(function(r){return r.text()}).then(function(h){p.srcdoc=h}).catch(function(x){})}else{p.src=p.src}};if(CONTENT){var tmr;ta.addEventListener("input",function(){clearTimeout(tmr);tmr=setTimeout(prev,400)});prev()}var chk=async function(){if(!NAME)return;var t="";try{t=await (await fetch(V+"err?data")).text()}catch(x){}var c=$("cerr");if(t){st("error",false);if(c){c.textContent=t;c.className="err"}}else{st(CONTENT?"saved":"compiled ok",true);if(c){c.textContent="compiled ok";c.className="ok"}prev()}};$("save").onclick=async function(){var name=NAME||($("pname")?$("pname").value.trim():"");if(!name){st("name required",false);return}if(MKDIR){st("creating...",true);var rf=await fetch("/apps/lattice/folder-new?name="+encodeURIComponent(name),{method:"POST"});if(!rf.ok){st("create failed "+rf.status,false);return}location="/apps/lattice/edit?into="+encodeURIComponent(name);return}st("saving...",true);var r=await fetch("/apps/lattice/page-save?name="+encodeURIComponent(name)+(NAME?"":"&new=1")+(CONTENT?"&type="+KIND:""),{method:"POST",body:ta.value});if(r.status===409){st("that page already exists",false);return}if(!r.ok){st("save failed "+r.status,false);return}if(!NAME){location="/apps/lattice/edit?name="+encodeURIComponent(name)+(CONTENT?"&kind="+KIND:"");return}st(CONTENT?"saved":"compiling...",true);setTimeout(chk,800);setTimeout(chk,2000)};window.addEventListener("keydown",function(e){if((e.metaKey||e.ctrlKey)&&e.key==="s"){e.preventDefault();$("save").onclick()}});var cs=$("csend");if(cs){var run=async function(){var c=$("cmd").value;if(!c)return;await fetch("/apps/lattice/page-cmd?name="+encodeURIComponent(NAME),{method:"POST",body:"cmd="+encodeURIComponent(c)});$("cmd").value="";setTimeout(prev,600)};cs.onclick=run;$("cmd").addEventListener("keydown",function(e){if(e.key==="Enter")run()})}document.querySelectorAll(".share button").forEach(function(b){b.onclick=async function(){var m=b.getAttribute("data-m");await fetch("/apps/lattice/page-share?name="+encodeURIComponent(NAME)+"&mode="+m,{method:"POST"});document.querySelectorAll(".share button").forEach(function(x){x.className=x.getAttribute("data-m")===m?"on":""});$("cwurl").innerHTML=m==="clearweb"?`<p>public: <a href="/apps/lattice/c/${NAME}" target="_blank">/c/${NAME}</a></p>`:"";setTimeout(prev,500)}});var d=$("del");if(d){d.onclick=async function(){if(!confirm("delete "+NAME+"?"))return;await fetch("/apps/lattice/page-del?name="+encodeURIComponent(NAME),{method:"POST"});location="/apps/lattice"}};var K2={md:"md",gmi:"gmi",html:"html",htm:"html",txt:"text",js:"js",css:"css",hoon:"hoon"};var seg=function(x){return x.toLowerCase().replace(/[^a-z0-9._~-]+/g,"-").replace(/^[-.]+|[-.]+$/g,"")};var upP=$("uppanel");var upM=$("upmsg");var upF=$("upfill");var upE=$("uperr");var upShow=function(){if(upP)upP.style.display="block";if(upE)upE.textContent="";if(upF)upF.style.width="0%"};var upProg=function(done,total,name){if(upM)upM.textContent="uploading "+done+"/"+total+(name?": "+name:"");if(upF)upF.style.width=Math.round(done*100/Math.max(total,1))+"%"};var uploadItems=async function(items){var list=[];var dirs={};var skipped=0;for(var i=0;i<items.length;i++){var rel=items[i].rel;var dot=rel.lastIndexOf(".");var ext=dot>0?rel.slice(dot+1).toLowerCase():"";var kind=K2[ext];if(!kind){skipped++;continue}var stem=dot>0?rel.slice(0,dot):rel;var parts=stem.split("/").map(seg).filter(function(x){return x});if(INTO)parts=INTO.split("/").concat(parts);var name=parts.join("/");if(!name){skipped++;continue}list.push({f:items[i].file,name:name,kind:kind});var pp=name.split("/");pp.pop();for(var j=1;j<=pp.length;j++)dirs[pp.slice(0,j).join("/")]=1}if(!list.length){upShow();if(upM)upM.textContent="no supported files (md gmi html txt js css hoon)";return}upShow();upProg(0,list.length,"");if(skipped&&upE)upE.textContent="skipped "+skipped+" unsupported; ";var dl=Object.keys(dirs).sort();for(var k=0;k<dl.length;k++){try{await fetch("/apps/lattice/folder-new?name="+encodeURIComponent(dl[k]),{method:"POST"})}catch(e){}}var fails=0;for(var m=0;m<list.length;m++){upProg(m,list.length,list[m].name);var txt=await list[m].f.text();var r=null;try{r=await fetch("/apps/lattice/page-save?name="+encodeURIComponent(list[m].name)+"&type="+list[m].kind,{method:"POST",body:txt})}catch(e){}if(!r||!r.ok){fails++;if(upE)upE.textContent+="failed: "+list[m].name+(r?" ("+r.status+")":"")+"; "}}upProg(list.length,list.length,"");if(upM)upM.textContent=fails?("done with "+fails+" failures"):("uploaded "+list.length+" files - reloading");if(!fails)setTimeout(function(){location.reload()},900)};var fromFileList=function(fl){var out=[];for(var i=0;i<fl.length;i++)out.push({file:fl[i],rel:fl[i].webkitRelativePath||fl[i].name});return out};var uf=$("upf");var ufb=$("upfb");if(uf&&ufb){ufb.onclick=function(e){e.preventDefault();uf.click()};uf.onchange=function(){if(uf.files.length)uploadItems(fromFileList(uf.files))}}var udd=$("upd");var udb=$("updb");if(udd&&udb){udb.onclick=function(e){e.preventDefault();udd.click()};udd.onchange=function(){if(udd.files.length)uploadItems(fromFileList(udd.files))}}var walkEnt=function(entry,path,out){return new Promise(function(res){if(entry.isFile){entry.file(function(f){out.push({file:f,rel:path+f.name});res()},function(){res()})}else if(entry.isDirectory){var rd=entry.createReader();var subs=[];var step=function(){rd.readEntries(function(es){if(!es.length){Promise.all(subs).then(function(){res()});return}for(var i=0;i<es.length;i++)subs.push(walkEnt(es[i],path+entry.name+"/",out));step()},function(){res()})};step()}else res()})};var trEl=document.querySelector(".tree");window.addEventListener("dragover",function(e){e.preventDefault();if(trEl)trEl.classList.add("dragover")});window.addEventListener("dragleave",function(e){if(trEl&&!e.relatedTarget)trEl.classList.remove("dragover")});window.addEventListener("drop",function(e){e.preventDefault();if(trEl)trEl.classList.remove("dragover");var its=e.dataTransfer&&e.dataTransfer.items;if(!its||!its.length)return;var out=[];var ps=[];for(var i=0;i<its.length;i++){var en=its[i].webkitGetAsEntry&&its[i].webkitGetAsEntry();if(en)ps.push(walkEnt(en,"",out))}Promise.all(ps).then(function(){if(out.length)uploadItems(out)})});var hlEl=$("hl");if(ta&&hlEl){var BS=String.fromCharCode(92);var QC=String.fromCharCode(39);var NLC=String.fromCharCode(10);var BT=String.fromCharCode(96);var F3=BT+BT+BT;var ehl=function(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;")};var RX=function(p,f){return new RegExp(p,f||"g")};var rules=[];if(KIND==="js")rules=[["c",RX("/"+BS+"*[^]*?"+BS+"*/")],["c",RX("//[^"+NLC+"]*")],["s",RX(`"[^"`+NLC+`]*"`)],["s",RX(QC+"[^"+QC+NLC+"]*"+QC)],["s",RX(BT+"[^"+BT+"]*"+BT)],["k",RX(BS+"b(function|return|var|let|const|if|else|for|while|new|typeof|async|await|class|try|catch|throw|this|null|true|false)"+BS+"b")],["n",RX(BS+"b[0-9][0-9a-fA-Fx.]*")]];if(KIND==="css")rules=[["c",RX("/"+BS+"*[^]*?"+BS+"*/")],["s",RX(`"[^"]*"`)],["s",RX(QC+"[^"+QC+"]*"+QC)],["n",RX("#[0-9a-fA-F]{3,8}")],["t",RX("[.][-a-zA-Z0-9_]+")],["k",RX("[-a-zA-Z]+(?=[:])")]];if(KIND==="html")rules=[["c",RX("<!--[^]*?-->")],["t",RX("</?[a-zA-Z][^>]*>?")]];if(KIND==="md")rules=[["c",RX(F3+"[^]*?("+F3+"|$)")],["k",RX("^#{1,6}[^"+NLC+"]*","gm")],["s",RX(BT+"[^"+BT+NLC+"]*"+BT)],["t",RX(BS+"[[^"+BS+"]"+NLC+"]*"+BS+"][(][^)"+NLC+"]*[)]")],["c",RX("^>[^"+NLC+"]*","gm")],["k",RX("^ {0,3}[-*+] ","gm")]];if(KIND==="gmi")rules=[["c",RX(F3+"[^]*?("+F3+"|$)")],["k",RX("^#{1,3}[^"+NLC+"]*","gm")],["t",RX("^=>[^"+NLC+"]*","gm")],["c",RX("^>[^"+NLC+"]*","gm")],["k",RX("^[*] ","gm")]];if(KIND==="hoon")rules=[["c",RX("::[^"+NLC+"]*")],["s",RX(QC+"[^"+QC+"]*"+QC)],["s",RX(`"[^"`+NLC+`]*"`)],["k",RX("^ *([+][+]|[+][$]) +[a-z][-a-z0-9]*","gm")],["t",RX("%[a-z][-a-z0-9]*")],["n",RX(BS+"b[0-9][0-9.a-fx]*")]];var hlRender=function(){var text=ta.value;if(!rules.length){hlEl.textContent=text+NLC;return}var out="";var i=0;while(i<text.length){var bi=-1;var bm=null;var bc=null;for(var r=0;r<rules.length;r++){rules[r][1].lastIndex=i;var m=rules[r][1].exec(text);if(m&&(bi<0||m.index<bi)){bi=m.index;bm=m[0];bc=rules[r][0]}}if(bi<0){out+=ehl(text.slice(i));break}out+=ehl(text.slice(i,bi))+"<span class=tk-"+bc+">"+ehl(bm)+"</span>";i=bi+(bm.length||1)}hlEl.innerHTML=out+NLC};var hlSync=function(){hlEl.scrollTop=ta.scrollTop;hlEl.scrollLeft=ta.scrollLeft};ta.addEventListener("input",hlRender);ta.addEventListener("scroll",hlSync);hlRender();hlSync()}})();</script>'
  ==
