...
So a rebuild of R rows in C chunks costs ~C x O(R), and — worse — saving a single
...
1. **A grub write costs `sham` (jam+sha256) over the containing directory's whole
   file map, at every ancestor up to root** — `+record-trees` -> `+put-tree`,
...
Fanout 256 — inside the "low hundreds" rule from (2), and small enough that a
whole-index rewrite is one bole.
...
`@p`), because the UI badges every hit by scope. No separate doc registry: the
key is the identity, and scope is a field, never part of the key — keying on
`[scope key]` would mint a second entry on a private->public flip and index the
document twice.
