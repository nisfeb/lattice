...
//  failure. Every scenario asserts both content and timing.
const check = makeCheck({ detailOnPass: true });

const launch = () => launchBrowser({ profile: PROFILE });
let browser = await launch();
let p = await openPage(browser, env);
let navs = 0;
...
try {
  ...
  navs = 0;
  const p2 = await openPage(browser, env);
  ...
  t = await nav(PAGE);
  navs = 0;
  const p3 = await openPage(browser, env);
  ...
  //  A throw above must not leave the chromium holding the profile: close and
  //  sweep on both paths.
