...
#
# Fix (publisher-side): the agent grows its home page at the empty spur
# (state-4 `home-cards`), so a remote home keen resolves. The home page is the
# authored /index, else the generated index. RED before the PUBLISHER runs
# the fix. GREEN after.
#
# Run against the fakes (consumer ~tyr fetching publisher ~zod):
#   SUB_COOKIE='urbauth-~tyr=…' ./scripts/remote-home-repro.sh
set -u
SUB_URL="${SUB_URL:-http://localhost:8082}"      # consumer ship (does the fetch), the ~tyr public port
