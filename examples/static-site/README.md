...
| `site.hoon` | `site/index` | hoon | the builder, composing the content into a themed index |
...
which produces the **public** `/c/…` URL. The `/x` explorer path is
owner-gated and would 403 for a visitor:
...
- **Standalone clearweb**: nested `/c/` paths and the chrome-less
  ...
  and `%share-tree` to publish/unpublish a whole subtree.
