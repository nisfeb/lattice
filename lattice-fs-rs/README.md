...
The **lattice** projection gives full page semantics, read-write. The **generic**
projection covers any other nexus / ball tree, read + overwrite + `rm`.
...
- **Fresh within 5 s.** External edits (browser, another client) appear on the
  next filesystem access after a 5 s TTL poll, on both transports. (The change
...
- **Lattice mode is fully read-write.** `ls`, `cat`, `rg`, `nvim`+`:w`, create,
  ...
  errors come via `lattice-fs errors <page>`.
...
Both transports verified end-to-end on the harness, with a full mount matrix
