...
++  migrate-8-9
  ...
  ==
::  +migrate-9-10: state-9 → state-10 — carry every field forward and add the
::  (empty) catalog-sweep slot. Pure, so on-load's upgrade is testable. The
::  catalog schema poke that pairs with this migration is fired from the
::  agent's +on-load card list (not here, since this is bowl-independent).
++  migrate-9-10
  |=  s=state-9
  ^-  state-10
  ...
  ==
::  +migrate-10-11: state-10 → state-11 — carry every field forward and add
::  the (empty) in-flight catalog-walks map. Pure, so on-load's upgrade is
::  testable.
++  migrate-10-11
  |=  s=state-10
  ^-  state-11
  :*  %11  content.s  published.s  pending.s  subs.s  fetches.s
      ...
      catalog-sweep.s  ~
  ==
::  +migrate-11-12: state-11 → state-12 — carry every field forward and add
::  the (empty) sequential sweep queue. Pure, so on-load's upgrade is testable.
++  migrate-11-12
  |=  s=state-11
  ^-  state-12
  :*  %12  content.s  published.s  pending.s  subs.s  fetches.s
      manifest.s  home.s  browse.s  know.s  trash.s  oquery.s
      catalog-sweep.s  catalog-walks.s  ~
  ==
::
